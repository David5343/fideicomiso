...
@section('content')
    <p class="text-end">
        <a href="{{ route('humanos.bancos.index') }}" class="btn btn-secondary" role="button">Lista de Bancos</a>
    </p>
    <div class="card mt-1" style="border-color:#333333">
        ...
        @endif
            <form action="{{ url('humanos/estados/' . $estado->id) }}" method="POST">
                @method('PUT')
                @csrf
                <div class="row mb-3">
                    ...
                    <div class="col-sm-2">
                        <input type="text" class="form-control form-control-lg" id="clave" name="clave"
                            value="{{ $estado->key }}" minlength="3" maxlength="5" required>
                        @error('clave')
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-6">
                        <input type="text" class="form-control form-control-lg" id="nombre" name="nombre"
                            value="{{ $estado->name }}" minlength="2" maxlength="40" required>
                        @error('nombre')
                            <br><small style="color: red">{{ $message }}</small>
                        @enderror
                    </div>
